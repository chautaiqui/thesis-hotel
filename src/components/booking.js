    ...
    if (tbody) {
      tbody.childNodes.forEach((item) => {
        item.childNodes.forEach((i) => {
          if (
            i.style.background === "rgb(127, 255, 0)" ||
    ...
    return {
      onClick: (event) => {
        ...
        _b.childNodes.forEach((item) => {
          if (
            item.firstElementChild.firstElementChild.innerText === choose.room
          )
            _p = item;
        });
    ...
    <div>
      <Table
        ...
        columns={dcol.map((i, idx) => {
          ...
          return {
            ...i,
            ...{ onCell: onCell, title: col[idx - 1], dataIndex: col[idx - 1] },
          };
        })}
        dataSource={room.map((item) => {
          return {
            ...bookingtoDate(item.bookings),
            room: item.name,
            key: item._id,
          };
        })}
